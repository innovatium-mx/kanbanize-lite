        ...
        <div>
            <Dropdown isOpen={dropdown} toggle={openCloseDropdown}  >
                        ...
                        <DropdownMenu className={dashboard.menuOptions}>
                            {Array.isArray(data) && data.map((element: any)=> (element.type === 0 || element.type === 1) && <DropdownItem className={dashboard.item} key={element.key} value={ element.workspace_id || element.workflow_id } onClick={() => onToggleClick(element.workspace_id || element.workflow_id )}>{element.name}</DropdownItem>)}
                        </DropdownMenu>
            </Dropdown>
        </div>
        </>
